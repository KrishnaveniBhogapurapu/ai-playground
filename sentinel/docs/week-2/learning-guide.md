...
| --- | --- | --- | --- |
| Call Claude using an official Python or TypeScript SDK | **Complete** | [`package.json`](../../package.json#L19), [`src/index.ts`](../../src/index.ts) | I used the official TypeScript Claude Agent SDK and its `query()` async generator. This is the accepted SDK approach for Sentinel. |
| Understand the Messages API request and response structure | **Complete through the accepted Agent SDK abstraction** | [`src/index.ts`](../../src/index.ts), [`src/index.ts`](../../src/index.ts), [`src/index.ts`](../../src/index.ts) | I handled Agent SDK messages that expose Messages API content blocks, raw stream events, tool use, tool results, final results, stop reasons, and usage. A separate Client SDK implementation is not required for this project. |
| Produce and validate structured JSON output | **Complete** | [`src/schemas/incident-analysis.ts`](../../src/schemas/incident-analysis.ts), [`src/index.ts`](../../src/index.ts), [`src/validation/incident-analysis.ts`](../../src/validation/incident-analysis.ts) | I converted the Week 1 response shape into TypeScript interfaces and JSON Schema, requested API-supported structured output, and validated the returned value again with Ajv. |
| Handle streaming and common API failures | **Complete through the accepted Agent SDK approach** | [`src/index.ts`](../../src/index.ts), [`src/index.ts`](../../src/index.ts), [`src/errors/classify-sdk-failure.ts`](../../src/errors/classify-sdk-failure.ts) | I displayed Agent SDK partial events, rejected interrupted partial output, and mapped common failures into application categories. |
| Send text and image inputs | **Complete** | [`src/index.ts`](../../src/index.ts), [`src/index.ts`](../../src/index.ts), [`experiments/week-2/multimodal/multimodal-evidence-run-1.json`](../../experiments/week-2/multimodal/multimodal-evidence-run-1.json#L99) | I supported text prompts and base64 image content blocks. I validated the image extension and required incident text before sending the multimodal request. |
| Select a model and thinking configuration | **Complete for Week 2** | [`src/config/reasoning.ts`](../../src/config/reasoning.ts), [`src/index.ts`](../../src/index.ts), [`experiments/week-2/thinking-comparison/comparison.md`](../../experiments/week-2/thinking-comparison/comparison.md#L5) | I selected a model alias, compared disabled thinking with adaptive thinking at high effort, and documented the untested modes and model-tier trade-offs. Week 2 does not require a broad benchmark or an experiment for every mode. |
| Track tokens, latency, and estimated cost | **Complete for post-request usage** | [`src/index.ts`](../../src/index.ts), [`experiments/week-2/request-metadata-run-1.json`](../../experiments/week-2/request-metadata-run-1.json#L1) | I recorded input/output/cache token usage, latency, cost, model usage, estimated thinking tokens, and stop reason. I did not call the token-counting endpoint before a request. |
| Understand and experiment with prompt caching | **Complete** | [`src/index.ts`](../../src/index.ts), [`experiments/week-2/prompt-caching/comparison.md`](../../experiments/week-2/prompt-caching/comparison.md#L9) | I kept a stable system contract, changed only the incident input, and observed cache creation followed by a cache read. I learned that caching is transparent request optimization, not memory. |
...
| Preview how Claude requests tools | **Complete** | `src/tools/incident-metrics.ts`, [`src/index.ts`](../../src/index.ts), [`experiments/week-2/tool-use/lifecycle-notes.md`](../../experiments/week-2/tool-use/lifecycle-notes.md#L3) | I exposed one read-only application tool, recorded Claude's request, validated its input, executed application code, returned the result, and validated the final analysis. |

The working application meets the main safety outcome: it returns either a validated incident analysis or an explicit typed failure. The accepted-output path is in [`src/index.ts`](../../src/index.ts), and the typed rejection contract is in [`src/errors/sentinel-failure.ts`](../../src/errors/sentinel-failure.ts).

## 2. Incident scenario and Sentinel milestone

### Incident scenario

I used the fictional `INC-104` checkout incident throughout the structured-output, thinking, caching, multimodal, and tool-use exercises. The tool's fixed fictional metric is defined in `src/tools/incident-metrics.ts`.

### Application flow

| Milestone stage | Status | Implementation | What it means |
| --- | --- | --- | --- |
| Incident text or dashboard image | **Complete** | Text reaches [`createIncidentAnalysisPrompt()`](../../src/prompts/incident-analysis.ts); image input is built in [`createImagePrompt()`](../../src/index.ts). | Sentinel accepts either typed incident evidence or an image plus incident description. |
| Validated application input | **Partial** | Empty image path/text and unsupported image types are rejected in [`src/index.ts`](../../src/index.ts) and [`src/index.ts`](../../src/index.ts). | Image input has explicit checks. Plain text is trimmed and required to be non-empty, but there is no separate text-input schema, size limit, or incident-ID contract. |
| Claude Messages API | **Complete through the accepted Agent SDK abstraction** | The request goes through Agent SDK `query()` in [`src/index.ts`](../../src/index.ts). | The SDK owns the lower-level API call while exposing Messages API content blocks, stream events, results, stop reasons, and usage to Sentinel. |
| Streamed or complete response | **Complete through the accepted Agent SDK approach** | Mode selection is in [`src/index.ts`](../../src/index.ts); partial events are handled at [`src/index.ts`](../../src/index.ts). | Stream mode displays partial events through `includePartialMessages`. Complete mode suppresses them and waits for the final Agent SDK result. |
| Structured-output validation | **Complete** | Output format is requested at [`src/index.ts`](../../src/index.ts), then validated at [`src/index.ts`](../../src/index.ts). | API-supported structure and application validation form two separate boundaries. |
| Accepted analysis or typed failure | **Complete** | Acceptance is printed at [`src/index.ts`](../../src/index.ts); failures are formatted in [`src/errors/sentinel-failure.ts`](../../src/errors/sentinel-failure.ts). | A partial, malformed, or schema-invalid value is never represented as an accepted analysis. |
| Model, prompt, token, latency, and cache metadata | **Complete for new runs** | Metadata is printed at [`src/index.ts`](../../src/index.ts), including prompt fields at [`src/index.ts`](../../src/index.ts). | I record the model, prompt contract/version, usage, latency, cost, cache counters, and stop reason. Original experiment records remain unchanged and therefore predate the prompt-version field. |
...
- [`query({ prompt, options })`](../../src/index.ts)
- [`includePartialMessages`](../../src/index.ts)
- [`stream_event` handling](../../src/index.ts)
...
- The stable evidence rules are in [`incidentAnalysisInstructions`](../../src/prompts/incident-analysis.ts).
- Multimodal classification rules are in [`multimodalEvidenceInstructions`](../../src/prompts/incident-analysis.ts).
- The incident input is wrapped separately in [`createIncidentAnalysisPrompt()`](../../src/prompts/incident-analysis.ts).
- These are passed as `systemPrompt` and `prompt` in [`runTurn()`](../../src/index.ts).
...
- Text requests use a string prompt at [`src/index.ts`](../../src/index.ts).
- Image requests create a user message containing both a text block and a base64 image block at [`src/index.ts`](../../src/index.ts).
- Tool requests and results are observed as `tool_use` and `tool_result` blocks at [`src/index.ts`](../../src/index.ts) and [`src/index.ts`](../../src/index.ts).
...
The requested model is read from `CLAUDE_MODEL`, with `sonnet` as the fallback at [`src/index.ts`](../../src/index.ts). The exact canonical models used are recorded in each run's `modelUsage`, for example [`direct-run-1-metadata.json`](../../experiments/week-2/thinking-comparison/direct-run-1-metadata.json#L1).
...
Sentinel records `message.stop_reason` at [`src/index.ts`](../../src/index.ts). Recorded examples include `end_turn` in [`request-metadata-run-1.json`](../../experiments/week-2/request-metadata-run-1.json#L5) and `tool_use` in [`tool-use-run-1.json`](../../experiments/week-2/tool-use/tool-use-run-1.json#L127).
...
The application records input, output, cache-creation, cache-read, model usage, latency, and cost at [`src/index.ts`](../../src/index.ts). I use response usage as measured evidence rather than estimating token totals from visible text.

### Synchronous and asynchronous requests

**Status: Asynchronous complete; synchronous not applicable in this TypeScript implementation.**

The Agent SDK returns an async generator, consumed with `for await` in [`src/index.ts`](../../src/index.ts). TypeScript network APIs are asynchronous; a non-streaming response still returns a Promise and is not a synchronous blocking API in the Python sense.
...
Environment loading begins with [`import 'dotenv/config'`](../../src/index.ts). Sentinel checks `CLAUDE_CODE_OAUTH_TOKEN` at [`src/index.ts`](../../src/index.ts), and `.env` is excluded by the repository [`.gitignore`](../../../.gitignore#L3). A safe template is stored in [`.env.example`](../../.env.example#L1).
...
The normal CLI sends incident evidence at [`src/index.ts`](../../src/index.ts), validates the final structured output, and prints it at [`src/index.ts`](../../src/index.ts).
...
Stream mode sets `includePartialMessages` and renders `stream_event` deltas at [`src/index.ts`](../../src/index.ts) and [`src/index.ts`](../../src/index.ts). A saved structured result from stream mode is [`api-structured-output-stream-run-1.json`](../../experiments/week-2/structured-output/api-structured-output-stream-run-1.json#L1).

### Failure outcomes

| Required outcome | Status | Implementation and evidence | Learning |
| --- | --- | --- | --- |
| Invalid input | **Complete** | Image type/read failures are raised in [`src/index.ts`](../../src/index.ts); missing image/text is rejected at [`src/index.ts`](../../src/index.ts). | Input failures occur before model-output validation. |
| Authentication or configuration | **Implemented and tested; no saved live failure run** | Missing token check: [`src/index.ts`](../../src/index.ts). Authentication mapping: [`classify-sdk-failure.ts`](../../src/errors/classify-sdk-failure.ts). Test: [`sentinel.test.ts`](../../tests/analysis.test.ts). | Configuration errors and API authentication errors belong to a different category from model-output failures. |
| Rate limit | **Tested only** | Mapping: [`classify-sdk-failure.ts`](../../src/errors/classify-sdk-failure.ts). Test: [`sentinel.test.ts`](../../tests/analysis.test.ts). | A 429 is an integration failure; retry policy is separate from content validation. |
| Timeout | **Tested only** | Mapping: [`classify-sdk-failure.ts`](../../src/errors/classify-sdk-failure.ts). Test: [`sentinel.test.ts`](../../tests/analysis.test.ts). | A timeout is a runtime outcome even if Claude might still be generating remotely. |
| Interrupted stream | **Complete with deterministic boundary evidence** | Real guard: [`turn-acceptance.ts`](../../src/application/turn-acceptance.ts). CLI integration: [`src/index.ts`](../../src/index.ts). Partial-then-abort test: [`sentinel.test.ts`](../../tests/analysis.test.ts). Evidence note: [`interrupted-stream-acceptance-test.md`](../../experiments/week-2/failures/interrupted-stream-acceptance-test.md#L1). | The test records partial output, aborts, attempts to submit an otherwise valid final analysis, verifies `interrupted-stream`, and confirms that accepted state remains false. |
| Context-limit failure | **Tested only** | Mapping: [`classify-sdk-failure.ts`](../../src/errors/classify-sdk-failure.ts). Test: [`sentinel.test.ts`](../../tests/analysis.test.ts). | Context limit is an input-size boundary, not an unsafe-content result. |
| Malformed response | **Complete** | Parser: [`parse-incident-analysis.ts`](../../src/validation/incident-analysis.ts). Test: [`sentinel.test.ts`](../../tests/analysis.test.ts). Invalid artifact: [`prompt-requested-json-run-2-invalid.txt`](../../experiments/week-2/structured-output/prompt-requested-json-run-2-invalid.txt#L1). | JSON parsing answers only whether the syntax is valid. |
| Schema-invalid output | **Complete** | Validator: [`parse-incident-analysis.ts`](../../src/validation/incident-analysis.ts). Tests: [`sentinel.test.ts`](../../tests/analysis.test.ts). | Schema validation checks required fields, types, enums, and unexpected properties. |

### Interrupted stream boundary

The first Ctrl+C aborts the active request at [`src/index.ts`](../../src/index.ts). [`TurnAcceptanceGuard`](../../src/application/turn-acceptance.ts) refuses acceptance whenever the abort signal is set. A result becomes accepted only through `acceptValidatedResult()` after schema validation; `assertCompleted()` rejects a turn that never reached that state.
...
The contract in [`IncidentAnalysis`](../../src/schemas/incident-analysis.ts) represents:
...
The equivalent JSON Schema begins at [`incidentAnalysisSchema`](../../src/schemas/incident-analysis.ts).

### Asking for JSON through a prompt

**Status: Complete as an experiment.**

The valid prompt-requested response is stored in [`prompt-requested-json-run-1-formatted.json`](../../experiments/week-2/structured-output/prompt-requested-json-run-1-formatted.json#L1). The invalid response is stored in [`prompt-requested-json-run-2-invalid.txt`](../../experiments/week-2/structured-output/prompt-requested-json-run-2-invalid.txt#L1).
...
I defined strict object schemas with required fields and `additionalProperties: false` in [`src/schemas/incident-analysis.ts`](../../src/schemas/incident-analysis.ts). This makes unexpected output a validation failure rather than silently ignoring it.

### API-supported structured output

**Status: Complete through the Agent SDK.**

The schema is passed through `outputFormat: { type: 'json_schema' }` at [`src/index.ts`](../../src/index.ts). Complete and stream-mode examples are saved in [`experiments/week-2/structured-output`](../../experiments/week-2/structured-output).

### Parsing JSON successfully

**Status: Complete.**

`parseIncidentAnalysis()` uses `JSON.parse()` and converts syntax errors into `malformed-json` at [`src/validation/incident-analysis.ts`](../../src/validation/incident-analysis.ts).

### Validating the structure

**Status: Complete.**

Ajv compiles the schema once at [`src/validation/incident-analysis.ts`](../../src/validation/incident-analysis.ts). A parsed value is accepted only after `validateIncidentAnalysisValue()` succeeds at [`src/validation/incident-analysis.ts`](../../src/validation/incident-analysis.ts).
...
The prompt instructs Claude to separate facts, assumptions, and hypotheses at [`src/prompts/incident-analysis.ts`](../../src/prompts/incident-analysis.ts). Multimodal and tool-use outputs explicitly classify or discuss unsupported content in [`multimodal-evidence-run-1.json`](../../experiments/week-2/multimodal/multimodal-evidence-run-1.json#L99) and [`lifecycle-notes.md`](../../experiments/week-2/tool-use/lifecycle-notes.md#L32).
...
| --- | --- | --- | --- |
| Create/use a fictional dashboard | **Complete** | [`incident-dashboard.png`](../../experiments/week-2/multimodal/incident-dashboard.png) | The image is a controlled fictional input rather than production evidence. |
| Send image with incident description | **Complete** | [`createImagePrompt()`](../../src/index.ts), [`/image` flow](../../src/index.ts) | The image and its accompanying text are distinct content blocks in one user message. |
| Record text observations | **Complete** | [`evidence_classification`](../../experiments/week-2/multimodal/multimodal-evidence-run-1.json#L99) | Direct text claims stay separate from visual observations. |
| Record image observations | **Complete** | [`image_observations`](../../experiments/week-2/multimodal/multimodal-evidence-run-1.json#L99) | A visible image value is still model-interpreted and must be checked against the actual image. |
| Record inferences | **Complete** | [`inferences`](../../experiments/week-2/multimodal/multimodal-evidence-run-1.json#L99) | Combining text and image information creates an inference, not a new direct observation. |
| Record unsupported information | **Complete** | [`unsupported_claims`](../../experiments/week-2/multimodal/multimodal-evidence-run-1.json#L99) | A convincing multimodal answer can still introduce claims absent from both sources. |

The multimodal contract is defined in [`EvidenceClassification`](../../src/schemas/incident-analysis.ts) and validated by [`validateMultimodalIncidentAnalysisValue()`](../../src/validation/incident-analysis.ts).
...
Direct mode sets `thinking: { type: 'disabled' }` at [`src/config/reasoning.ts`](../../src/config/reasoning.ts). Its analysis and metadata are saved in [`direct-run-1-analysis.json`](../../experiments/week-2/thinking-comparison/direct-run-1-analysis.json#L1) and [`direct-run-1-metadata.json`](../../experiments/week-2/thinking-comparison/direct-run-1-metadata.json#L1).
...
Thinking mode sets `thinking: { type: 'adaptive' }` at [`src/config/reasoning.ts`](../../src/config/reasoning.ts). The run recorded approximately 1,200 thinking tokens in [`thinking-run-1-metadata.json`](../../experiments/week-2/thinking-comparison/thinking-run-1-metadata.json#L32).
...
Thinking mode sets `effort: 'high'` at [`src/config/reasoning.ts`](../../src/config/reasoning.ts). I did not compare low, medium, high, xhigh, or max effort levels.
...
The same difficult incident was run in direct and adaptive-thinking configurations. [`comparison.md`](../../experiments/week-2/thinking-comparison/comparison.md#L5) compares:
...
| --- | --- |
| Model | [`direct-run-1-metadata.json`](../../experiments/week-2/thinking-comparison/direct-run-1-metadata.json#L2) |
| Input tokens | [`direct-run-1-metadata.json`](../../experiments/week-2/thinking-comparison/direct-run-1-metadata.json#L30) |
| Output tokens | [`direct-run-1-metadata.json`](../../experiments/week-2/thinking-comparison/direct-run-1-metadata.json#L31) |
| Thinking tokens | [`thinking-run-1-metadata.json`](../../experiments/week-2/thinking-comparison/thinking-run-1-metadata.json#L32) |
| Maximum output tokens supported by model | [`thinking-run-1-metadata.json`](../../experiments/week-2/thinking-comparison/thinking-run-1-metadata.json#L25) |
| Latency | [`thinking-run-1-metadata.json`](../../experiments/week-2/thinking-comparison/thinking-run-1-metadata.json#L33) |
| Estimated cost | [`thinking-run-1-metadata.json`](../../experiments/week-2/thinking-comparison/thinking-run-1-metadata.json#L35) |
| Stop reason | [`thinking-run-1-metadata.json`](../../experiments/week-2/thinking-comparison/thinking-run-1-metadata.json#L36) |
...
The system contract is part of every independent request. Without caching, the model processes those tokens again. Sentinel keeps that contract stable at [`src/index.ts`](../../src/index.ts), allowing later requests to reuse a cached prefix.
...
The stable contract is appended through `systemPrompt` with dynamic sections excluded at [`src/index.ts`](../../src/index.ts). Each incident is a new `query()` call, so the cache experiment does not rely on session resume or conversation history.

### Paired requests

| Request | Analysis | Metadata |
| --- | --- | --- |
| Stable contract + Incident A | [`request-1-incident-a-analysis.json`](../../experiments/week-2/prompt-caching/request-1-incident-a-analysis.json#L1) | [`request-1-incident-a-metadata.json`](../../experiments/week-2/prompt-caching/request-1-incident-a-metadata.json#L30) |
| Same contract + Incident B | [`request-2-incident-b-analysis.json`](../../experiments/week-2/prompt-caching/request-2-incident-b-analysis.json#L1) | [`request-2-incident-b-metadata.json`](../../experiments/week-2/prompt-caching/request-2-incident-b-metadata.json#L30) |
...
The calculations and caveats are documented in [`comparison.md`](../../experiments/week-2/prompt-caching/comparison.md#L9).
...
The experiment is recorded in [`configuration-and-session-comparison.md`](../../experiments/week-2/claude-code/configuration-and-session-comparison.md#L31). A continued session remembered `SENTINEL-104`; a clean session did not. Both loaded the project settings and `CLAUDE.md`.

### Loaded instructions and settings

**Status: Complete.**

I inspected `/status`, `/context`, and `/permissions`. The shared project settings, one memory file for `CLAUDE.md`, allowed build/validation commands, and denied `.env` read rule are recorded in [`configuration-and-session-comparison.md`](../../experiments/week-2/claude-code/configuration-and-session-comparison.md#L3).

### Why `CLAUDE.md` is not a security boundary

**Status: Complete.**

`CLAUDE.md` is context interpreted by the model. It guides choices but cannot guarantee compliance. Claude Code evaluates permission rules before covered tool calls, while stronger boundaries require sandboxing, operating-system permissions, credential isolation, and application checks. This distinction is documented at [`configuration-and-session-comparison.md`](../../experiments/week-2/claude-code/configuration-and-session-comparison.md#L70).
...
| --- | --- | --- |
| Application sends a tool definition | `tool() definition`; registration in [`runTurn()`](../../src/index.ts) | The application defines the tool name, description, input schema, annotations, and handler. Claude sees only what the application exposes. |
| Claude returns `tool_use` | [`src/index.ts`](../../src/index.ts); [`tool-use-run-1.json`](../../experiments/week-2/tool-use/tool-use-run-1.json#L3) | Claude requested a tool with structured arguments and a unique ID. The request did not execute the operation. |
| Application validates and executes | Zod input shape at `incident-metrics.ts`; handler at `incident-metrics.ts` | The SDK validates the schema before entering the handler. Application code controls the in-memory lookup. |
| Application returns `tool_result` | [`src/index.ts`](../../src/index.ts); [`tool-use-run-1.json`](../../experiments/week-2/tool-use/tool-use-run-1.json#L22) | The result references the original tool-use ID and can be marked as an error or success. |
| Claude continues the response | Structured analysis in [`tool-use-run-1.json`](../../experiments/week-2/tool-use/tool-use-run-1.json#L33) | Claude used the retrieved metric in its analysis, but the final output still required schema validation and evidence review. |
...
- **What application code executed:** `readIncidentMetric()` at `incident-metrics.ts`.
- **What result returned:** 9 percent, observed at 10:04 UTC, sourced from the fictional Sentinel monitoring snapshot.

The detailed first-person observations are in [`lifecycle-notes.md`](../../experiments/week-2/tool-use/lifecycle-notes.md#L3). The implementation goes slightly beyond a paper preview because the Agent SDK completed the tool loop, but the exposed tool remains narrow, read-only, local, and fictional.
...
| 2. Reject interrupted stream | **Complete** | The actual CLI acceptance guard is exercised by a partial-output-then-abort test and documented in [`interrupted-stream-acceptance-test.md`](../../experiments/week-2/failures/interrupted-stream-acceptance-test.md). |
| 3. One valid and one invalid structured response | **Complete** | [`experiments/week-2/structured-output`](../../experiments/week-2/structured-output) |
| 4. One fictional dashboard image | **Complete** | [`incident-dashboard.png`](../../experiments/week-2/multimodal/incident-dashboard.png) |
| 5. Classify failure categories | **Complete** | [`sentinel-failure.ts`](../../src/errors/sentinel-failure.ts), [`sentinel.test.ts`](../../tests/analysis.test.ts) |
| 6. Compare direct and thinking/effort | **Complete** | [`experiments/week-2/thinking-comparison/comparison.md`](../../experiments/week-2/thinking-comparison/comparison.md#L1) |
| 7. Record tokens, latency, stop reason, cost | **Complete** | [`direct-run-1-metadata.json`](../../experiments/week-2/thinking-comparison/direct-run-1-metadata.json#L30) |
| 8. Basic prompt-caching experiment | **Complete** | [`experiments/week-2/prompt-caching/comparison.md`](../../experiments/week-2/prompt-caching/comparison.md#L1) |
...
| 11. Annotated `tool_use` and `tool_result` | **Complete** | [`experiments/week-2/tool-use/lifecycle-notes.md`](../../experiments/week-2/tool-use/lifecycle-notes.md#L1) |
...
| Structured-output schema and validator | **Complete** | [`src/schemas/incident-analysis.ts`](../../src/schemas/incident-analysis.ts), [`src/validation/incident-analysis.ts`](../../src/validation/incident-analysis.ts) |
| Multimodal request | **Complete** | [`experiments/week-2/multimodal`](../../experiments/week-2/multimodal) |
| Typed failure categories | **Complete** | [`src/errors/sentinel-failure.ts`](../../src/errors/sentinel-failure.ts) |
| Model and thinking comparison | **Complete for direct versus adaptive thinking** | [`experiments/week-2/thinking-comparison`](../../experiments/week-2/thinking-comparison) |
| Token, latency, and cost record | **Complete** | [`experiments/week-2/request-metadata-run-1.json`](../../experiments/week-2/request-metadata-run-1.json#L1) |
| Prompt-caching experiment | **Complete** | [`experiments/week-2/prompt-caching`](../../experiments/week-2/prompt-caching) |
...
| Tool-use lifecycle notes | **Complete** | [`experiments/week-2/tool-use/lifecycle-notes.md`](../../experiments/week-2/tool-use/lifecycle-notes.md) |
| Debugging report | **Complete** | [`experiments/week-2/debugging-report.md`](../../experiments/week-2/debugging-report.md) |
...
| Distinguish API failure from unsafe model content | **Complete** | API failures are classified in [`src/errors`](../../src/errors); unsupported content is discussed in [`lifecycle-notes.md`](../../experiments/week-2/tool-use/lifecycle-notes.md#L32). |
...
| Estimate basic request cost | **Complete using SDK-reported cost** | Each metadata file records `total_cost_usd`; [`comparison.md`](../../experiments/week-2/thinking-comparison/comparison.md) verifies one total by summing the recorded per-model estimates without inventing historical prices. |
