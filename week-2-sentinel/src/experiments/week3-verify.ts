...
import { Investigation, fictionalProvider, type ToolRequest } from '../runtime/investigation.js';
import { runCustomInvestigation } from '../runtime/custom-loop.js';
import { createEvidenceTools } from '../tools/evidence-tools.js';
import { metricInput, healthInput, investigationPrompt, injectionProvider, scriptedTransport, scriptedFinal, scriptedAnalysis, toolResponse } from './week3-fixtures.js';
...
try {
  for (const [id, name, input, index] of [
    ...
    await sdk.preToolUse({ hook_event_name: 'PreToolUse', session_id: 'offline', transcript_path: '', cwd: process.cwd(), tool_name: `mcp__sentinel__${name}`, tool_input: input, tool_use_id: id }, id, { signal: sdk.investigation.signal });
...
const directory = 'experiments/week-3';
