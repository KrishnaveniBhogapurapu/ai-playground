...
import { Investigation, fictionalProvider } from '../runtime/investigation.js';
...
import { metricInput, healthInput, investigationPrompt, injectedLog, injectionProvider, scriptedFinal, toolResponse } from '../experiments/week3-fixtures.js';
...
test('SDK pre-hook blocks unknown, malformed and unauthorized requests; approved calls execute once', async () => {
  ...
  const callHook = (id: string, name: string, input: unknown) => tools.preToolUse({
