...
| --- | --- | --- | --- |
| `ThinkingConfig` | A type-safe union for disabled, adaptive, or fixed-budget thinking configurations. | Stored in [`ReasoningConfig`](../src/config/reasoning.ts#L10). | Direct response, extended thinking, and adaptive thinking concepts. |
| `EffortLevel` | A type-safe union of supported effort names such as `low`, `medium`, and `high`. | The thinking mode uses `high` at [`src/config/reasoning.ts`](../src/config/reasoning.ts#L31). | Thinking/effort comparison. |
...
| Import | Meaning | Sentinel usage | Week 2 mapping |
| --- | --- | --- | --- |
| `SDKResultSuccess` | The SDK's normal result shape. It can still contain `is_error: true` and an API status when the loop produced a result frame for an API failure. | Checked in [`classifySdkFailure()`](../src/errors/classify-sdk-failure.ts#L10). | Authentication, rate-limit, timeout, API, usage, and stop handling. |
...
| Import | Meaning | Sentinel usage | Week 2 mapping |
| --- | --- | --- | --- |
| `tool` | Creates a custom SDK tool from a name, description, Zod input shape, async handler, and optional annotations. | Defines `get_incident_metric` at [`src/tools/incident-metrics.ts`](../src/tools/incident-metrics.ts#L89). | Tool definition, input validation, controlled execution, and tool result. |
| `createSdkMcpServer` | Packages one or more in-process tools as an MCP server that the Agent SDK can expose to Claude. | Creates the `sentinel` server at [`src/tools/incident-metrics.ts`](../src/tools/incident-metrics.ts#L129). | Tool-use lifecycle preview. |
...
Normal CLI text becomes a string prompt at [`src/index.ts`](../src/index.ts#L475):
...
An image request uses the async generator at [`src/index.ts`](../src/index.ts#L69):
...
The value comes from [`readReasoningConfig()`](../src/config/reasoning.ts#L16):
...
| Runtime schema validation | Does the actual returned JavaScript value satisfy Sentinel's contract? | Ajv in [`src/validation/parse-incident-analysis.ts`](../src/validation/parse-incident-analysis.ts#L43). |
...
The definition starts at [`src/tools/incident-metrics.ts`](../src/tools/incident-metrics.ts#L89):
...
The classifier is at [`src/errors/classify-sdk-failure.ts`](../src/errors/classify-sdk-failure.ts#L10).
